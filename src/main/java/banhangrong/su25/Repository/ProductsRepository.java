...
public interface ProductsRepository extends JpaRepository<Products, Long> {
    ...
    @Query("""
       ...
       JOIN CategoriesProducts cp
         ON cp.id.productId = p.productId
       WHERE LOWER(p.status) = LOWER(:status)
         AND cp.id.categoryId = :categoryId
""")
    ...
    @Query("""
       ...
       JOIN CategoriesProducts cp
         ON cp.id.productId = p.productId
       WHERE cp.id.categoryId = :categoryId
""")
...
""")
    ...
    // Sử dụng pessimistic write lock để tránh race condition khi 2 người cùng mua hết sản phẩm
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT p FROM Products p WHERE p.productId = :productId")
    Optional<Products> findByIdWithLock(@Param("productId") Long productId);

    boolean existsBySellerIdAndNameIgnoreCase(Long sellerId, String nm);
}
