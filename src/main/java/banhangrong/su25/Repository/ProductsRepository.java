...
public interface ProductsRepository extends JpaRepository<Products, Long> {
    ...
    // Rank (1-based, dense) of a seller by revenue across all sellers (MySQL-compatible, no window functions)
    @Query(value = "SELECT 1 + COUNT(DISTINCT t.revenue) AS seller_rank\n" +
            "FROM (\n" +
            "  SELECT p2.seller_id, COALESCE(SUM(oi2.price_at_time * oi2.quantity),0) AS revenue\n" +
            "  FROM products p2 LEFT JOIN order_items oi2 ON oi2.product_id = p2.product_id\n" +
